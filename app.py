...
@app.route("/ownerhome")
def owner():
    return render_template("Owner/owner.html", variable=ownerEmail, residents=len(id_list), workers=len(worker_List),
                           totalincome=totalexpenditures(), totalsalary=totalsalaries())
...
def ownerVerfiy():
    ...
    if verifyowner(MyEmail, password):
        return render_template("Owner/owner.html", variable=ownerEmail, residents=len(id_list),
                               workers=len(worker_List), totalincome=totalexpenditures(), totalsalary=totalsalaries())
    else:
        return render_template("Owner/loginOwner.html")
...
def manager():
    return render_template("Manager/manager.html", variable=managerEmail, totalresidents=len(id_list),
                           totalworkers=len(worker_List), totalfoods=len(food_List), types=len(room_List))
...
def Workersadd():
    ...
    worker = workers(name, job, str(salary))
    worker_List.append(worker)
    saveWorkerData()
    return render_template("Manager/manager.html", variable=managerEmail, totalresidents=len(id_list),
                           totalworkers=len(worker_List), totalfoods=len(food_List), types=len(room_List))
...
def resident():
    ...
    return render_template("Resident/resident.html", variable=MyEmail, roomtype=MyObject.roomType,
                           roomno=MyObject.roomNo, expen=MyObject.expenditures, yourcomplains=yourcomplains)
